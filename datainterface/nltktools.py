...
Classes:
  * DictNltk: save an imported datasource (in a list of dicts) to NLTK files,
    ...
    def __init__(self, nltkfolder, dictitems, tkey):
        ...
        Arguments:
          ...
          * tkey: a string which is a key in each of the dicts in dictitems,
                  ...
                  dict must be a dict itself, with a '_text' key (containing a
                  string) and, if that string is not empty, an additional
    ...
    def save_files(self):
        ...
        Effects:
          * looks at the self.tkey key in each dict of dictitem (this will
            ...
            that key must be itself a dict. It then looks at the '_text' key
            in that sub-dict: if it's not empty, it takes the '_text_stripped'
            key in that same dict and saves that to the file indicated by the
            'nltk_filename' key of the 1st-level dict.
