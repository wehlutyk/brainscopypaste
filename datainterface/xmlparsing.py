...
Classes:
  * TextHtmlParser: extract links (<a>...</a> tags) from HTML strings
                    (subclass of HTMLParser)

Methods:
  * _ConvertXmlToDictRecurse: recursively convert an ElementTree Element to a
                              dict (not meant to be used directly)
  * ConvertXmlToDict: convert an XML file or an ElementTree Element to a dict
                      (this is a wrapper for _ConvertXmlToDictRecurse and is
...
class TextHtmlParser(HTMLParser):
    ...
    Effects: after parsing (use the 'feed' method from HTMLParser for that),
             ...
             per attribute found in the <a ...> declaration; if one attribute
...
def _ConvertXmlToDictRecurse(node, text_tags):
    ...
    Arguments:
      ...
      * text_tags: a list of strings representing which tags contain textual
                   data in the XML; those nodes are treated as HTML: in
    ...
    Returns: if the ElementTree Element had no subnodes, this returns a string
             ...
             name, and the corresponding value is the result of
             '_ConvertXmlToDictRecurse' on that node; if that tag was declared
             multiple times, the value corresponding to the tag is a list of
             results from _ConvertXmlToDictRecurse (one for each declaration);
             any text found out of subnodes if stored under the '_text' key of
             the resulting dict. If, in addition, the Element name (tag) is in
             'text_tags' (i.e. it contains textual data), then two other keys
             are added in the dict: '_text_stripped', which is the text
             stripped from HTML entities, and '_text_outlinks', which is a
             list of dicts, each one representing a link (<a>...</a> tag, but
             in HTML entity format) found in the text (the (key, value) pairs
             of those dicts are the attributes found in the <a ...>
