...
class SubstitutionsAnalyzer(object):
    ...
    def __init__(self, aa):
        ...
        self.aa = aa
        self.base_savefile_prefix = get_fileprefix(aa)
    ...
    def analyze(self, axss=None, filepaths=None):
        ...
        in `self.analyses` is run. Usually `axss` and `filepaths` are provided
        when calling this method from
        :meth:`SubstitutionsGroupAnalyzer.analyze`, to plot different analyses
        on the same axes. So when provided, this method will skip setting a
        title to the figures and saving the figures to files if asked to in the
        analysis args (that is done by
        :meth:`SubstitutionsGroupAnalyzer.analyze` in that case). If either of
        those two arguments is not provided, figures and axes are created
        locally, titles are added to the figures, and figures are saved to
        files if the analysis args say to do so.

        Parameters
        ----------
        axss : list of :class:`~matplotlib.axes.Axes` instances, optional
            The axes on which to plot; if not provided, axes are created.
        filepaths : list of strings, optional
            The filepaths to save the figures to; if not provided, filepaths
            are created according to the analysis args.

        See Also
        --------
        .base.AnalysisCase, SubstitutionsGroupAnalyzer.analyze

        """

        self.aa.print_analysis()

        # Populate figures and axes if they weren't given
        if axss is None or filepaths is None:
            figs_axes_are_own = True
            figs = []
            axss = []
            filepaths = []
            for a in self.analyses:
                figs.append(pl.figure())
                axss.append(a.build_axes(figs[-1]))
                filename = (self.base_savefile_prefix +
                            re.sub(' ', '-', a.savefile_postfix()))
                filepaths.append(st.mt_analysis_figure_file.format(filename))
        else:
            figs_axes_are_own = False
        ...
        # Do the title printing and figure saving ourselves if the
        # figures and axes were generated by us.
        if figs_axes_are_own:
            # Print titles
            for a, fig in zip(self.analyses, figs):
                a.print_fig_text(fig, self.aa.title())

            # Save if asked to
            if self.aa.save:
                for save, fig, filepath in zip(saves, figs, filepaths):
                    if save:
